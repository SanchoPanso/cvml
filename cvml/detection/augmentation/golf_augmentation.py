...
def tube_augmentation(img: np.ndarray, 
                      labels: np.ndarray, 
                      augmenter: MaskMixup) -> Tuple[np.ndarray]:
    height, width = img.shape[0]
    tube_img, tube_labels = get_tube_crop(img, labels, height, width)
    new_tube_img, new_tube_labels = augmenter(tube_img, tube_labels)
