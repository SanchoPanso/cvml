...
from cvml.annotation.bounding_box import BoundingBox, CoordinatesType
from cvml.annotation.annotation import Annotation


def read_coco(path: str) -> Annotation:
    """
    ...
    classes = _get_classes_from_coco(input_data)
    images = _get_images_from_coco(input_data)
    bboxes = _get_bboxes_from_coco(input_data)
    ...
    for bbox_id in bboxes.keys():
        bbox = get_bounding_box_from_coco_data(bbox_id, classes, images, bboxes)
...
def write_coco(annotation: Annotation, path: str, image_ext: str = '.jpg'):
    """
    ...
    categories = _get_categories_from_annotation(annotation)
    images = _get_images_from_annotation(annotation, image_ext)
    annotations = _get_bboxes_from_annotation(annotation)
...
def read_yolo(path: str, img_size: tuple, classes: List[str] = None, data_yaml_path: str = None) -> Annotation:
    """_summary_
...
def _get_classes_from_coco(coco_dict: dict) -> dict:
...
def _get_images_from_coco(coco_dict: dict) -> dict:
...
def _get_bboxes_from_coco(coco_dict: dict) -> dict:
...
def get_bounding_box_from_coco_data(bbox_id: str, classes: dict, images: dict, bboxes: dict) -> BoundingBox:
...
def _get_images_from_annotation(annotation: Annotation, image_ext: str) -> dict:
