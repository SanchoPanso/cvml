...
def main():
    ...
    save_dir = '/home/student2/datasets/prepared/tmk_number_yolo_640px_05022023' #'/home/student2/datasets/prepared/TMK_CVS3_0701'
    ...
    for dataset_dir in raw_dirs:
        ...
        preprocess_fn = lambda x: cv2.resize(expo(x, 15), (640, 535))
        ...
        annotation_data = AnnotationEditor.change_classes_by_new_classes(annotation_data, cls_names)
    ...
    ## number_december_1 train
    dataset_dir = '/home/student2/Downloads/number_december1_reannot/train'
    
    image_dir = os.path.join(dataset_dir, 'images')
    all_files = glob.glob(os.path.join(image_dir, '*'))
    image_files = list(glob.glob(os.path.join(image_dir, '*')))
    print(len(image_files), dataset_dir)

    preprocess_fn = lambda x: cv2.resize(x, (640, 535))
    image_sources = convert_paths_to_single_sources(paths=image_files,
                                                    preprocess_fn=preprocess_fn)

    annotation_path = os.path.join(dataset_dir, 'annotations', 'data.json')
    renamer = lambda x: x
    ...
    ## number_december_1 valid
    dataset_dir = '/home/student2/Downloads/number_december1_reannot/valid'
    ...
    if create_compressed_samples:
        if os.name == 'posix':
            all_samples = list(split_proportions.keys())
